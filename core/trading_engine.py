...
class TradingEngine:
    ...
    def _evaluate_and_enter(self, stock_code: str, stock_name: str):
        ...
        if self._is_entry_cooldown():
            logger.info(
                f"진입 대기(쿨다운 {trading_settings.order_cooldown_sec}초): {stock_name}"
            )
            return

        # 과도한 슬리피지 방지 (매수 1호가 기준)
        if self._is_slippage_too_high(current_price, orderbook, side="BUY"):
            logger.warning(f"슬리피지 초과로 진입 보류: {stock_name}")
            return
    ...
    def force_close_all_positions(self, reason: str = "장 마감 전 강제 청산"):
        """모든 포지션 시장가 일괄 청산"""
        for _, pos in list(self.position_mgr.positions.items()):
            try:
                price_data = self.market.get_current_price(pos.stock_code)
                ...
                logger.error(f"강제 청산 실패 ({pos.stock_name}): {e}")
    ...
    def _is_entry_cooldown(self) -> bool:
        """연속 주문 방지를 위한 진입 쿨다운 체크"""
        if self.last_order_time <= 0:
            return False
        return (time.time() - self.last_order_time) < trading_settings.order_cooldown_sec

    @staticmethod
    def _is_slippage_too_high(current_price: int, orderbook: dict, side: str = "BUY") -> bool:
        """1호가 기준 예상 슬리피지 체크"""
        if current_price <= 0:
            return True
        ...
        try:
            if side == "BUY":
                best_price = int(orderbook.get("asks", [{}])[0].get("price", 0))
            else:
                best_price = int(orderbook.get("bids", [{}])[0].get("price", 0))
        ...
        slippage_pct = abs(best_price - current_price) / current_price * 100
        return slippage_pct > trading_settings.max_slippage_pct
