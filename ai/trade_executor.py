...
class ClaudeTradeExecutor:
    ...
    def __init__(self):
        ...
        if ai_settings.anthropic_api_key:
            ...
            logger.warning("ANTHROPIC_API_KEY 미설정: Claude 매매 판단은 HOLD로 처리됩니다.")
        self.model = ai_settings.claude_model
...
"""
    ...
    def _normalize_decision(data: dict) -> dict:
        ...
        return {
            ...
            "risk_assessment": str(data.get("risk_assessment", "정보 없음"))[:500],
            "suggested_qty": int(data.get("suggested_qty", 0) or 0),
            "suggested_stop_loss": int(data.get("suggested_stop_loss", 0) or 0),
            "suggested_take_profit": int(data.get("suggested_take_profit", 0) or 0),
        }
